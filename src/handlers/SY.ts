...
export async function takeSYSnapshot(ctx: EthContext): Promise<ShareMapping> {
  const allUsers = (await db.asyncFind<AccountSnapshot>({})).map((x) => x._id);
  const allSYBalances = await readAllUserERC20Balances(ctx, allUsers, PENDLE_POOL_ADDRESSES.SY);
